...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "grpc")]
    {
        ...
        tonic_prost_build::configure()
            .build_server(true)
            .build_client(true)
            // Map nexus.core.FileMetadata to raft's generated core module.
            // (After the trim-fork merge into nexus-cluster, raft's old
            // `transport::proto` submod now lives at crate::raft::transport::proto.)
            .extern_path(".nexus.core", "crate::raft::transport::proto::nexus::core")
            .out_dir(std::env::var("OUT_DIR")?)
