...
use crate::raft::prelude::{Command, FullStateMachine, ZoneConsensus};
...
/// ``nexus_vfs_core::kernel::MetaStore`` impl backed by a single ``ZoneConsensus``.
///
...
impl MetaStore for ZoneMetaStore {
    ...
    fn put(&self, path: &str, mut metadata: KernelFileMetadata) -> Result<(), MetaStoreError> {
        ...
        match result {
            ...
            crate::raft::prelude::CommandResult::Error(e) => {
    ...
    fn append_stream_entry(&self, key: &str, data: &[u8]) -> Result<(), MetaStoreError> {
        ...
        match result {
            crate::raft::prelude::CommandResult::Success => Ok(()),
            crate::raft::prelude::CommandResult::Error(e) => Err(MetaStoreError::IOError(format!(
...
mod tests {
    use super::*;
    use crate::raft::raft::ZoneRaftRegistry;
