...
use crate::raft::blob_fetcher::BlobFetcherSlot;
use crate::raft::raft::{
...
fn proto_command_to_internal(proto: RaftCommand) -> Option<Command> {
    match proto.command? {
        ...
        ProtoCommandVariant::AcquireLock(al) => Some(Command::AcquireLock {
            ...
            mode: crate::raft::prelude::LockMode::Exclusive,
...
/// and stepping logic.
async fn parse_and_step_message<S: crate::raft::raft::StateMachine + Send + Sync + 'static>(
    ...
    async fn step_message(
        ...
        let node = match self.registry.get_node(&req.zone_id) {
            ...
            None => {
                ...
                let root_peers = self
                    .registry
                    .get_peers(nexus_vfs_core::contracts::ROOT_ZONE_ID)
                    .ok_or_else(|| {
...
impl ZoneApiService for ZoneApiServiceImpl {
    ...
    async fn delete_zone(
        ...
        if zone_id == nexus_vfs_core::contracts::ROOT_ZONE_ID {
            return Ok(Response::new(DeleteZoneResponse {
                success: false,
...
impl WitnessZoneRegistry {
    ...
    fn current_auto_join_peers(&self) -> Vec<NodeAddress> {
        if let Some(root) = self.zones.get(nexus_vfs_core::contracts::ROOT_ZONE_ID) {
    ...
    fn setup_witness_zone(
        &self,
        zone_id: &str,
        config: crate::raft::raft::RaftConfig,
        mut peers: Vec<NodeAddress>,
        runtime_handle: &tokio::runtime::Handle,
    ) -> Result<ZoneConsensus<WitnessStateMachine>> {
        use crate::raft::raft::RaftStorage;
        use crate::raft::transport::{ClientConfig, RaftClientPool, TransportLoop};
        use raft::Storage;
