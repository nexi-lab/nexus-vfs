...
use crate::raft::transport::{NodeAddress, RaftClientPool, SharedPeerMap};
...
struct ForwardContext {
    ...
    cached_api_client:
        std::sync::Arc<tokio::sync::Mutex<Option<(String, crate::raft::transport::RaftApiClient)>>>,
}
...
impl<S: StateMachine + 'static> ZoneConsensus<S> {
    ...
    async fn forward_to_leader(&self, command: Command) -> Result<CommandResult> {
        ...
        if let Some(ctx) = &self.forward_ctx {
            ...
            return match crate::raft::transport::forward_propose(
                &ctx.client_pool,
                &leader_addr,
                command,
                &ctx.zone_id,
...
impl ZoneConsensus<super::state_machine::FullStateMachine> {
    ...
    pub fn advisory_state_blocking(
        &self,
    ) -> Arc<parking_lot::Mutex<crate::raft::raft::state_machine::LockState>> {
        Arc::clone(
...
mod tests {
    use super::*;
    use crate::raft::raft::state_machine::{FullStateMachine, WitnessStateMachine};
