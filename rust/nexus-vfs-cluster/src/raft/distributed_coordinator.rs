...
use crate::raft::transport::NodeAddress;
use crate::raft::zone_meta_store::ZoneMetaStore;
use crate::raft::{TlsFiles, ZoneManager};
...
fn attempt_join_zone_round(
    ...
    for peer in peer_addrs {
        ...
        loop {
            ...
            let attempt = join_runtime.block_on(crate::raft::transport::call_join_zone_rpc(
                &endpoint,
                zone_id,
                node_id,
                self_address,
...
impl RaftDistributedCoordinator {
    ...
    pub fn init_from_env(&self, kernel: &Kernel) -> CoordinatorResult<bool> {
        ...
        // `nexus_vfs_core::util::transport_primitives::hostname_to_node_id`) — they
...
impl DistributedCoordinator for RaftDistributedCoordinator {
    ...
    fn metastore_for_zone(
        ...
        let store: Arc<dyn MetaStore> = Arc::new(crate::raft::zone_meta_store::ZoneMetaStore::new(
            consensus,
            runtime,
            mount_point,
        ));
    ...
    fn create_zone(&self, kernel: &Kernel, zone_id: &str) -> CoordinatorResult<()> {
        ...
        while Instant::now() < deadline {
            ...
            for peer in &candidates {
                ...
                let fut = crate::raft::transport::call_join_zone_rpc(
                    &peer.endpoint,
                    zone_id,
                    self_id,
                    &self_address,
    ...
    fn remove_zone(&self, _kernel: &Kernel, zone_id: &str, force: bool) -> CoordinatorResult<()> {
        ...
        for (parent_zone_id, mount_path, _global) in &mounts {
            if let Some(parent) = zm.registry().get_node(parent_zone_id) {
                if let Err(e) =
                    crate::raft::zone_manager::propose_delete_metadata(runtime, &parent, mount_path)
                {
                    if !force {
                        return Err(format!(
    ...
    fn join_cluster(
        ...
        let mut redirected_once = false;
        loop {
            let attempt = runtime.block_on(crate::raft::transport::call_join_zone_rpc(
                &endpoint,
                zone_id,
                self_id,
                &self_address,
    ...
    fn share_zone(
        ...
        let route = kernel
            .vfs_router_arc()
            .route(local_path, nexus_vfs_core::contracts::ROOT_ZONE_ID)
...
fn reconstruct_global_path(
    ...
    if parent_zone_id == nexus_vfs_core::contracts::ROOT_ZONE_ID || parent_zone_id.is_empty() {
...
fn wire_mount_core(
    vfs_router: &Arc<nexus_vfs_core::kernel::core::vfs_router::VFSRouter>,
    lock_manager: &Arc<nexus_vfs_core::kernel::core::lock::LockManager>,
    registry: &Arc<crate::raft::raft::ZoneRaftRegistry>,
    ...
    let root_canonical = canonicalize("/", nexus_vfs_core::contracts::ROOT_ZONE_ID);
    ...
    let canonical = canonicalize(&global_path, nexus_vfs_core::contracts::ROOT_ZONE_ID);
    ...
    if !lock_manager.locks_installed() {
        match registry.get_node(nexus_vfs_core::contracts::ROOT_ZONE_ID) {
            Some(root_consensus) => {
                tracing::info!(
...
fn unwire_mount_core(
    ...
    if let Some(global) = unwired_global {
        vfs_router.remove(&global, nexus_vfs_core::contracts::ROOT_ZONE_ID);
    }
}
...
fn install_mount_apply_cb_impl(
    ...
    cross_zone_mounts: &Arc<DashMap<String, Vec<CrossZoneMountTuple>>>,
    parent_zone_id: &str,
    consensus: &crate::raft::raft::ZoneConsensus<crate::raft::raft::FullStateMachine>,
) {
    ...
    use crate::raft::raft::MountApplyEvent;
...
pub fn install(kernel: &Arc<Kernel>) -> Result<(), String> {
    ...
    crate::raft::blob_fetcher_handler::install(kernel);
    Ok(())
}
