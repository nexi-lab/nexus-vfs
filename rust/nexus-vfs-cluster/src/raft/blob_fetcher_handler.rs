...
use crate::raft::blob_fetcher::BlobFetcher;

use nexus_vfs_core::kernel::kernel::OperationContext;
use nexus_vfs_core::kernel::vfs_router::VFSRouter;
...
/// `nexus_vfs_core::kernel::python::register` so the raft server's `BlobFetcherSlot`
...
pub fn install(kernel: &Arc<nexus_vfs_core::kernel::kernel::Kernel>) {
    let Some(any_slot) = kernel.take_pending_blob_fetcher_slot() else {
        return;
    };
    let slot = match any_slot.downcast::<crate::raft::blob_fetcher::BlobFetcherSlot>() {
        Ok(boxed) => *boxed,
        Err(_) => {
            tracing::error!(
    ...
    let lookup = kernel.content_id_lookup_fn(nexus_vfs_core::contracts::ROOT_ZONE_ID);
