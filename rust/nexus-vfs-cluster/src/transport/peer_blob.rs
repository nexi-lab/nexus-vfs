...
pub struct PeerBlobClient {
    ...
    tls: parking_lot::RwLock<Option<nexus_vfs_core::util::transport_primitives::TlsConfig>>,
}
...
impl PeerBlobClient {
    ...
    /// cert.
    pub fn install_tls_config(&self, tls: nexus_vfs_core::util::transport_primitives::TlsConfig) {
    ...
    async fn channel_for(&self, address: &str) -> Result<tonic::transport::Channel, String> {
        ...
        let client_cfg = nexus_vfs_core::util::transport_primitives::ClientConfig {
            tls,
            ..Default::default()
        };
        let channel =
    ...
    pub(crate) async fn fetch_async(
        ...
        // SSOT: ``nexus_vfs_core::contracts::MAX_GRPC_MESSAGE_BYTES``.
        let mut client = ZoneApiServiceClient::new(channel)
            .max_decoding_message_size(nexus_vfs_core::contracts::MAX_GRPC_MESSAGE_BYTES)
            .max_encoding_message_size(nexus_vfs_core::contracts::MAX_GRPC_MESSAGE_BYTES);
...
// ── HAL trait wiring ─────────────────────────────────────────────────
//
// Kernel holds an `Arc<dyn nexus_vfs_core::kernel::hal::peer::PeerBlobClient>`. The
...
impl nexus_vfs_core::kernel::hal::peer::PeerBlobClient for PeerBlobClient {
    ...
    fn install_tls(&self, ca_pem: &[u8], cert_pem: Option<&[u8]>, key_pem: Option<&[u8]>) {
        ...
        // which the underlying `nexus_vfs_core::util::transport_primitives::TlsConfig`
        ...
        PeerBlobClient::install_tls_config(
            self,
            nexus_vfs_core::util::transport_primitives::TlsConfig {
...
pub fn install(kernel: &nexus_vfs_core::kernel::kernel::Kernel) {
    let client = Arc::new(PeerBlobClient::new(Arc::clone(kernel.runtime())));
    kernel.set_peer_client(client as Arc<dyn nexus_vfs_core::kernel::hal::peer::PeerBlobClient>);
}
