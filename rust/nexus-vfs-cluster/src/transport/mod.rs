...
//!   implementing `nexus_vfs_core::util::transport_primitives::PeerBlobClient`,
...
/// Out-bound VFS gRPC client. Re-exported from `nexus_vfs_core::kernel::rpc_transport`
/// where the type is declared (kernel-internal `RemoteMetaStore`
/// wrappers wrap the same struct).
pub mod vfs {
    pub use nexus_vfs_core::kernel::rpc_transport::{RpcTransport, TlsConfig as VfsTlsConfig};
}

// Re-export low-level primitive types under the transport crate's
// namespace so existing call sites keep working.
pub use nexus_vfs_core::util::transport_primitives::{
...
pub type Result<T> = nexus_vfs_core::util::transport_primitives::Result<T>;
