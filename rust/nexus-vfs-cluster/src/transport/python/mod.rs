...
//!
//! Mirrors `nexus_vfs_core::kernel::python::register`, `nexus_vfs_core::services::python::register`,
//! `nexus_vfs_core::backends::python::register` — single entry point the
...
use nexus_vfs_core::kernel::generated_kernel_abi_pyo3::PyKernel;
use pyo3::prelude::*;

use crate::transport::federation;
...
fn install_transport_wiring(kernel: PyRef<'_, PyKernel>) -> PyResult<()> {
    crate::transport::peer_blob::install(kernel.kernel_ref());
    Ok(())
}
