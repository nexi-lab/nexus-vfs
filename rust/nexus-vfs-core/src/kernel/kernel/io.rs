...
use crate::kernel::lock_manager::{LockManager, LockMode};
use crate::kernel::meta_store::{FileMetadata, DT_DIR, DT_MOUNT, DT_PIPE, DT_REG, DT_STREAM};
...
#[allow(dead_code)]
struct ResolvedRead {
    route: crate::kernel::core::vfs_router::RouteResult,
...
fn clone_read_result(
    ...
    req: &crate::kernel::kernel::ReadRequest,
...
impl Kernel {
    ...
    pub fn sys_read(
        &self,
        reqs: &[crate::kernel::kernel::ReadRequest],
...
struct WriteCommitInput<'a> {
    ...
    content: &'a [u8],
    offset: u64,
    route: &'a crate::kernel::vfs_router::RouteResult,
...
impl Kernel {
    ...
    pub fn sys_write(
        &self,
        reqs: &[crate::kernel::kernel::WriteRequest],
        ctx: &OperationContext,
            ...
            if entry.entry_type == DT_PIPE {
                if let Some(buf) = self.pipe_manager.get(path) {
                    match buf.push(effective_content) {
                        ...
                        Err(crate::kernel::pipe::PipeError::Full(_, _)) => {
                            // Full — return miss so Python async shell retries
                            return miss();
                        }
                        Err(crate::kernel::pipe::PipeError::Closed(msg)) => {
            ...
            if entry.entry_type == DT_STREAM {
                if let Some(buf) = self.stream_manager.get(path) {
                    match buf.push(effective_content) {
                        ...
                        Err(crate::kernel::stream::StreamError::Full(_, _)) => return miss(),
                        Err(crate::kernel::stream::StreamError::Closed(msg)) => {
    ...
    fn load_buffered_write_base(
        ...
        backend
            ...
            .map_err(|err| match err {
                crate::kernel::abc::object_store::StorageError::NotFound(_) => {
    ...
    pub fn sys_unlink(
        &self,
        reqs: &[crate::kernel::kernel::UnlinkRequest],
        ctx: &OperationContext,
    ...
    fn sys_unlink_single(
        ...
        let (parent_zone, _user_mp) =
            crate::kernel::vfs_router::extract_zone_from_canonical(&route.mount_point);
        ...
        if !dst_route.is_cas {
            ...
            let exists = dst_route.backend.as_ref().and_then(|b| {
                match b.get_content_size(&dst_route.backend_path) {
                    Ok(_) => Some(true),
                    Err(crate::kernel::abc::object_store::StorageError::NotFound(_)) => Some(false),
                    Err(_) => None,
                }
            });
    ...
    fn copy_via_read_write(
        &self,
        src_route: &crate::kernel::vfs_router::RouteResult,
        dst_route: &crate::kernel::vfs_router::RouteResult,
    ...
    fn ensure_parent_directories(
        ...
        loop {
            match cur.rfind('/') {
                ...
                Some(pos) => {
                    cur = &path[..pos];
                    if cur.is_empty() || cur == crate::contracts::VFS_ROOT {
                        break;
                    }
                    let exists = self
    ...
    fn sys_read_batch_impl(
        ...
        for (i, req) in reqs.iter().enumerate() {
            ...
            if let Some(m) = entry.as_ref() {
                if m.entry_type == crate::kernel::meta_store::DT_PIPE
                    || m.entry_type == crate::kernel::meta_store::DT_STREAM
                {
            ...
            if req.len == Some(0) {
                if let Some(m) = entry.as_ref() {
                    if m.entry_type != crate::kernel::meta_store::DT_LINK {
        ...
        if cap < usize::MAX {
            ...
            for i in 0..n {
                ...
                if meta.entry_type == crate::kernel::meta_store::DT_LINK {
                    results[i] = Some(Err(KernelError::IOError(format!(
    ...
    pub fn readdir(&self, parent_path: &str, zone_id: &str, is_admin: bool) -> Vec<(String, u8)> {
        ...
        let global_prefix = if normalized == crate::contracts::VFS_ROOT {
            crate::contracts::VFS_ROOT.to_string()
        } else {
            format!("{}/", normalized)
        };

        let needs_zone_filter = !is_admin && zone_id != crate::contracts::ROOT_ZONE_ID;
        ...
        let parent_for_join = if parent_path == crate::contracts::VFS_ROOT {
            ""
        } else {
            parent_path.trim_end_matches('/')
        };
    ...
    pub fn readdir_paged(
        ...
        if parent_path == crate::contracts::LOCKS_PATH_PREFIX
            || parent_path.starts_with(&format!("{}/", crate::contracts::LOCKS_PATH_PREFIX))
        {
            if !is_admin {
            ...
            let prefix = if parent_path == crate::contracts::LOCKS_PATH_PREFIX {
                ""
            } else {
                parent_path
                    .strip_prefix(&format!("{}/", crate::contracts::LOCKS_PATH_PREFIX))
                    .unwrap_or("")
            };
...
mod read_batch_tests {
    ...
    fn rreq(path: &str, offset: u64, len: Option<u64>) -> crate::kernel::kernel::ReadRequest {
        crate::kernel::kernel::ReadRequest {
            path: path.into(),
            offset,
            len,
            timeout_ms: 5000,
    ...
    fn read_batch_rejects_pipe_entry() {
        use crate::kernel::meta_store::{FileMetadata, DT_PIPE};
        let k = kernel_with_backend();
        let c = ctx();
