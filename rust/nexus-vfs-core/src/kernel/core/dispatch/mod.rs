...
impl FileEvent {
    ...
    /// results without `crate::kernel::` access to the raw field.
...
/// `crate::services::agents::status_resolver`) can impl this trait. Same
...
// `crate::services::rebac::RebacPermissionHook`), NOT in kernel.
...
///
/// `pub` so peer crates (crate::services::audit, crate::services::permission, …)
...
/// [`crate::contracts::SYSTEM_PATH_PREFIX`] (`/__sys__/`):
///
/// ```ignore
/// fn on_pre(&self, ctx: &HookContext) -> Result<HookOutcome, String> {
///     if crate::contracts::is_system_path(ctx.path()) {
