...
//! `ObjectStoreProvider`) live in `crate::kernel::hal::*`. ObjectStore
//! extension hooks (`LlmStreamingBackend`) live at the crate root
//! (`crate::kernel::llm_streaming`). Peer-blob fetch
//! (`crate::kernel::hal::peer::PeerBlobClient`) is a transport-layer
//! abstraction reached through the kernel's peer_client slot. Kernel
//! primitives (vfs_router, dlc, dcache, locks, dispatch, …) live in
//! `crate::kernel::core::*` and never declare traits.
//!
