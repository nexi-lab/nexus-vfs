...
pub trait ObjectStore: Send + Sync {
    ...
    fn as_cas(&self) -> Option<&crate::kernel::cas_engine::CASEngine> {
        None
    }
    ...
    ///
    /// Trait declaration lives at `crate::kernel::llm_streaming` — an
    /// ObjectStore extension hook, distinct from §3.B Control-Plane
    /// HAL traits in `crate::kernel::hal/`. Concrete connector impls live in
    /// `crate::backends::transports::api::ai::*`.
    fn as_llm_streaming(&self) -> Option<&dyn crate::kernel::llm_streaming::LlmStreamingBackend> {
        None
    }
