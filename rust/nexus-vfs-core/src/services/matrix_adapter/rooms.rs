...
use crate::services::matrix_adapter::auth::AuthSession;
use crate::services::matrix_adapter::error::AdapterError;
use crate::services::matrix_adapter::pdu::chat_envelope_to_pdu_event;
use crate::services::matrix_adapter::pdu::pdu_send_to_chat_envelope;
use crate::services::matrix_adapter::room_id::{decode_room_id, encode_room_id};
use crate::services::matrix_adapter::router::AdapterState;
...
pub(super) fn ctx_from_session(session: &AuthSession) -> crate::kernel::kernel::OperationContext {
    crate::kernel::kernel::OperationContext {
...
fn require_kernel<K: crate::kernel::abi::KernelAbi>(
...
pub async fn room_state<K: crate::kernel::abi::KernelAbi>(
...
pub async fn room_state_event<K: crate::kernel::abi::KernelAbi>(
    State(state): State<AdapterState<K>>,
    Extension(session): Extension<AuthSession>,
...
/// query.
pub async fn room_messages<K: crate::kernel::abi::KernelAbi>(
    State(state): State<AdapterState<K>>,
    Extension(session): Extension<AuthSession>,
    Path(room_id): Path<String>,
...
pub async fn joined_members<K: crate::kernel::abi::KernelAbi>(
    State(state): State<AdapterState<K>>,
    Extension(session): Extension<AuthSession>,
    Path(room_id): Path<String>,
) -> Result<Json<Value>, AdapterError> {
...
/// otherwise.
pub async fn room_send<K: crate::kernel::abi::KernelAbi>(
    State(state): State<AdapterState<K>>,
    Extension(session): Extension<AuthSession>,
    ...
    let result: crate::kernel::kernel::SysWriteResult = tokio::task::spawn_blocking(move || {
        kernel_for_write
...
/// path-based codec.
pub async fn create_room<K: crate::kernel::abi::KernelAbi>(
    State(state): State<AdapterState<K>>,
...
pub async fn room_join<K: crate::kernel::abi::KernelAbi>(
    State(state): State<AdapterState<K>>,
    Extension(session): Extension<AuthSession>,
    Path(room_id): Path<String>,
) -> Result<Json<Value>, AdapterError> {
...
pub async fn room_leave<K: crate::kernel::abi::KernelAbi>(
    State(state): State<AdapterState<K>>,
    Extension(session): Extension<AuthSession>,
    Path(room_id): Path<String>,
) -> Result<Json<Value>, AdapterError> {
...
/// `crate::services::services::managed_agent::proc_entry::create_dt_stream` — the two
/// callers (managed-agent spawn vs Matrix createRoom) share the same
/// DT_STREAM contract, just at different paths.
fn create_chat_stream<K: crate::kernel::abi::KernelAbi>(
