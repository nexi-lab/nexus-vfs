...
fn stamps_from_through_dispatch_after_install() {
    ...
    install_a2a_stamp_hook(&kernel).expect("install a2a stamp hook");
...
#[test]
fn empty_agent_id_passes_through_unrewritten() {
    // NoAuth bring-up: empty agent_id ⇒ the policy returns None ⇒ no
    // rewrite (behaviour-preserving). Fail-closed is a stage-2 change.
    let kernel = Arc::new(Kernel::new());
    install_a2a_stamp_hook(&kernel).expect("install a2a stamp hook");
    ...
    assert!(
        kernel
            ...
            .expect("hook must accept")
            .is_none(),
        "empty agent_id must not rewrite the envelope"
    );
}
