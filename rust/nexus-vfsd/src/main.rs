...
//!   * [`nexus_vfs_cluster::raft::ZoneManager`] (multi-zone Raft + gRPC server)
...
use nexus_vfs_cluster::raft::distributed_coordinator::{
...
async fn run_daemon(common: CommonArgs) -> Result<()> {
    ...
    let topology_handle = tokio::spawn(async move {
        loop {
            match zm_for_loop.apply_topology(nexus_vfs_core::contracts::ROOT_ZONE_ID) {
                Ok(true) => {
...
fn run_unmount(common: CommonArgs, mount_point: &str) -> Result<()> {
    let _bundle = open_zone_manager(&common, None)?;
    let kernel = Arc::new(Kernel::new());
    let ctx = nexus_vfs_core::contracts::OperationContext::new(
...
async fn run_join(
    ...
    tokio::task::spawn_blocking(move || {
        nexus_vfs_cluster::raft::distributed_coordinator::bootstrap_or_join_zone(
            zm_for_join.as_ref(),
            &zone_id_for_join,
            node_id,
            &self_addr_for_join,
