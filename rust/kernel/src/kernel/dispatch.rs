...
impl Kernel {
    ...
    pub fn bring_up_services(
        ...
        // so a post-boot runtime `enlist` auto-starts.
        //
        // This latch no longer gates any client path: the gRPC server holds
        // EVERY request until the whole data plane is ready, and a data plane
        // is only opened by a boot that has already come through here.
        self.service_registry.mark_bootstrapped();
        Ok(())
    }
