...
impl Kernel {
    ...
    // 8 per-syscall `federation_peer_X` wrappers) live in
    // `kernel/federation.rs` next to the `DistributedCoordinator`
    // slot accessors — see that file's module doc for the rationale
    // (per `docs/KERNEL-ARCHITECTURE.md` §3, federation-domain
    // Control-Plane HAL wiring belongs in `kernel/federation.rs`,
    // not in syscall-implementation files like io.rs).  Call sites
    // (`self.federation_peer_X(...)`) are unchanged.
}
