...
pub trait FederationPeerClient: Send + Sync {
    ...
    /// sys_rename (in-band error); callers here just relay it.
    ///
    /// Returns `Ok(())` when the peer completes the rename — the
    /// peer's metastore mutations (delete old key + put new key)
    /// run on its side and the raft apply replicates the result
    /// back to this voter.
    ...
    /// want to update.
    ///
    /// Returns `Ok(())` when the peer's sys_setattr completed —
    /// the peer's metastore.put + observe + post-hook dispatch
    /// run on its side and the raft apply replicates the row
    /// back to this voter.
