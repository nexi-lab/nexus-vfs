...
//!   * [`nexus_cluster::raft::ZoneManager`] (multi-zone Raft + gRPC server)
...
use nexus_cluster::raft::distributed_coordinator::{
    bootstrap_or_join_zone, read_or_mint_node_id, validate_bootstrap_mode,
    validate_peers_excludes_self, BootstrapMode,
};
use nexus_cluster::raft::federation::{parse_federation_env, ENV_FEDERATION_MOUNTS, ENV_FEDERATION_ZONES};
use nexus_cluster::raft::transport::{bootstrap_tls, NodeAddress};
use nexus_cluster::raft::{TlsFiles, ZoneManager};
...
async fn run_daemon(common: CommonArgs) -> Result<()> {
    ...
    let vfs_routes = nexus_cluster::transport::grpc::build_vfs_routes(
        Arc::clone(&kernel),
        vfs_auth,
        64 * 1024 * 1024,
        "nexusd-cluster",
...
async fn run_join(
    ...
    tokio::task::spawn_blocking(move || {
        nexus_cluster::raft::distributed_coordinator::bootstrap_or_join_zone(
            zm_for_join.as_ref(),
            &zone_id_for_join,
            node_id,
            &self_addr_for_join,
