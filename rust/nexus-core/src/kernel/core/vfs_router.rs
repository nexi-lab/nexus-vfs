...
use crate::kernel::abc::object_store::{ExternalTransport, ObjectStore, StorageError};
...
impl VFSRouter {
    ...
    pub fn route(&self, path: &str, zone_id: &str) -> Result<RouteResult, RouteError> {
        ...
        if zone_id != crate::contracts::ROOT_ZONE_ID {
            if let Some(result) = self.route_in_zone(path, crate::contracts::ROOT_ZONE_ID) {
                return Ok(result);
            }
        }
...
/// [`Kernel::zone_key`](crate::kernel::kernel::Kernel::zone_key): given the
...
mod tests {
    ...
    fn mount_points_for_coherence_key_finds_direct_and_crosslinks() {
        ...
        impl crate::kernel::meta_store::MetaStore for KeyedStub {
            fn get(
                &self,
                _: &str,
            ...
            fn put(
                &self,
                _: &str,
                _: crate::kernel::meta_store::FileMetadata,
            ) -> Result<(), crate::kernel::meta_store::MetaStoreError> {
                Ok(())
            }
            fn delete(&self, _: &str) -> Result<bool, crate::kernel::meta_store::MetaStoreError> {
                Ok(false)
            }
            fn list(
                &self,
