...
pub trait DistributedCoordinator: Send + Sync + 'static {
    ...
    /// federation state.
    fn list_zones(&self, kernel: &crate::kernel::kernel::Kernel) -> Vec<String>;
    ...
    fn is_initialized(&self, kernel: &crate::kernel::kernel::Kernel) -> bool {
...
impl DistributedCoordinator for NoopDistributedCoordinator {
    fn list_zones(&self, _kernel: &crate::kernel::kernel::Kernel) -> Vec<String> {
        Vec::new()
    }

    fn cluster_info(
        &self,
        _kernel: &crate::kernel::kernel::Kernel,
        _zone_id: &str,
    ) -> CoordinatorResult<ClusterInfo> {
