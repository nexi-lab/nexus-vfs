...
pub async fn upload<K: crate::kernel::abi::KernelAbi>(
    ...
    tokio::task::spawn_blocking(move || -> Result<(), AdapterError> {
        ...
        // MailboxStampingHook.
        let ctx = crate::kernel::kernel::OperationContext {
...
/// future concern).
pub async fn download<K: crate::kernel::abi::KernelAbi>(
    State(state): State<AdapterState<K>>,
    Extension(_session): Extension<AuthSession>,
    ...
    let (bytes, mime_type) = tokio::task::spawn_blocking(
        move || -> Result<(Vec<u8>, String), AdapterError> {
            ...
            let read = match read_result {
                Ok(r) => r,
                Err(crate::kernel::kernel::KernelError::FileNotFound(_)) => {
...
pub async fn thumbnail<K: crate::kernel::abi::KernelAbi>(
