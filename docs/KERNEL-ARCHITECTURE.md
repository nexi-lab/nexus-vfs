...
`<vfs-path>` must be non-`/`.  The boot-time `PathLocalBackend`
already owns that mount point, and `Kernel::add_mount`'s
`rebind_missing_backends` branch keys specifically on
`(zone="root", mount_point="/")` — overwriting that mount silently
re-points every backend-less federation child mount at the
operator's driver.

`<zone>` is operator-supplied without further constraint:

* `root` for the node-local "expose a host fs subtree through the VFS
  so other syscall-tier consumers (search, audit, agents) can see it"
  use case — same-canonical routing keeps the mount strictly local
  (the federation wire-mount path only fires when target zone differs
  from parent zone)
* a **separate raft zone** when the same mount needs to extend across
  peers via federation (§3.B.1 control-plane HAL)
...
is live; a separate zone the operator names at this point is minted
